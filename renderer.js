...
window.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('Button references:', { startVoiceBtn, stopVoiceBtn, listenOnceBtn, speakBtn, clearHistoryBtn, startEyeBtn, stopEyeBtn, typeModeBtn, keyModeBtn });
  ...
  if (typeModeBtn) {
    typeModeBtn.onclick = async () => {
      try {
        console.log('Type mode button clicked');
        const result = await window.voiceAPI.toggleTypeMode();
        console.log('Type mode toggle result:', result);
        if (result.success) {
          updateModeButton('type', result.enabled);
        } else {
          console.error('Failed to toggle type mode:', result.error);
        }
      } catch (error) {
  ...
  // Add manual command input handler for testing
  const commandBox = document.getElementById('commandBox');
  const sendCommandBtn = document.getElementById('sendCommandBtn');

  if (sendCommandBtn && commandBox) {
    sendCommandBtn.onclick = async () => {
      const command = commandBox.value.trim();
      if (command) {
        console.log('Sending manual command:', command);
        try {
          // Send command directly to voice server for processing
          const response = await fetch('http://localhost:5005/process-manual-command', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ text: command })
          });
          const result = await response.json();
          console.log('Manual command result:', result);

          // If it's a virtual keyboard command, handle it directly
          if (result.command && result.command.action === 'virtual-keyboard') {
            if (result.command.command === 'show-keyboard') {
              showVirtualKeyboard();
            } else if (result.command.command === 'hide-keyboard') {
              hideVirtualKeyboard();
            }
          }
        } catch (error) {
  ...
  if (keyModeBtn) {
    keyModeBtn.onclick = async () => {
      try {
        console.log('Key mode button clicked');
        const result = await window.voiceAPI.toggleKeyMode();
        console.log('Key mode toggle result:', result);
        if (result.success) {
          updateModeButton('key', result.enabled);
        } else {
          console.error('Failed to toggle key mode:', result.error);
        }
      } catch (error) {
        console.error('Error toggling key mode:', error);
      }
    };
  }
  ...
  // Test direct show button
  const testDirectShowBtn = document.getElementById('testDirectShowBtn');
  if (testDirectShowBtn) {
    testDirectShowBtn.onclick = () => {
      console.log('🎹 RENDERER: Test direct show button clicked');

      // Debug: Check if virtual keyboard element exists
      const keyboard = document.getElementById('virtualKeyboard');
      console.log('🎹 RENDERER: Keyboard element exists:', !!keyboard);
      if (keyboard) {
        console.log('🎹 RENDERER: Current keyboard display:', keyboard.style.display);
        console.log('🎹 RENDERER: Current keyboard classes:', keyboard.className);
      }

      showVirtualKeyboard();

      // Double-check after showing
      setTimeout(() => {
        if (keyboard) {
          console.log('🎹 RENDERER: After show - display:', keyboard.style.display);
          console.log('🎹 RENDERER: After show - classes:', keyboard.className);
        }
      }, 100);
    };
    console.log('Test direct show button handler added');
  }
  ...
  if (keyboardElement) {
    ...
    // Don't auto-show keyboard on startup
    // setTimeout(() => {
  ...
  // Initialize mode buttons to OFF state
  updateModeButton('type', false);
  updateModeButton('key', false);

  console.log('Initialization complete');
});

// Mode button update function (from this morning)
function updateModeButton(mode, enabled) {
  try {
    console.log(`Updating ${mode} mode button, enabled:`, enabled);

    if (mode === 'type') {
      const btn = document.getElementById('typeModeBtn');
      const indicator = document.getElementById('typeModeIndicator');
      const status = document.getElementById('typeModeStatus');

      if (btn) {
        btn.textContent = `Type Mode: ${enabled ? 'ON' : 'OFF'}`;
        btn.className = enabled ? 'mode-button active' : 'mode-button';
      }
      if (indicator) {
        indicator.className = enabled ? 'mode-indicator active' : 'mode-indicator inactive';
      }
      if (status) {
      ...
      const indicator = document.getElementById('keyModeIndicator');
      const status = document.getElementById('keyModeStatus');

      if (btn) {
        btn.textContent = `Key Mode: ${enabled ? 'ON' : 'OFF'}`;
        btn.className = enabled ? 'mode-button active' : 'mode-button';
      }
      if (indicator) {
        indicator.className = enabled ? 'mode-indicator active' : 'mode-indicator inactive';
    ...
    console.log(`${mode} mode button updated successfully`);
  } catch (error) {
    console.error(`Error updating ${mode} mode button:`, error);
  }
}

// Listen for mode status updates from main process (from this morning)
if (window.electronAPI) {
  window.electronAPI.on('mode-status-update', (data) => {
    console.log('Received mode status update:', data);
    updateModeButton(data.mode, data.enabled);

    // Show feedback if provided
    if (data.feedback) {
      console.log('Mode feedback:', data.feedback);
    }
  });
}
