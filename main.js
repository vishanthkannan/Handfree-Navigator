...
app.whenReady().then(() => {
  createWindow();
  // Ensure the voice server is started proactively on Windows
...
async function callVoiceServer(endpoint, method = 'GET', body = null) {
  try {
    ...
    // Suppress noisy errors for queue polling when server isn't up yet
    if (endpoint !== '/get-command-queue') {
...
ipcMain.handle('voice-send-command', async (event, command) => {
  try {
    ...
    const response = await callVoiceServer('/process-command', 'POST', { command: command });
    return response;
  } catch (error) {
...
// Mode control IPC handlers (from this morning)
ipcMain.handle('toggle-type-mode', async (event) => {
  try {
    // Send command to voice server to toggle type mode
    const response = await callVoiceServer('/process-manual-command', 'POST', {
      text: 'toggle type mode'
    });

    if (response && response.command) {
      // Forward the response to renderer for UI updates
      event.sender.send('mode-status-update', {
        mode: 'type',
        enabled: response.command.data?.enabled || false,
        feedback: response.command.feedback
      });
      return { success: true, enabled: response.command.data?.enabled || false };
    }

    return { success: false, error: 'Failed to toggle type mode' };
  } catch (error) {
...
ipcMain.handle('toggle-key-mode', async (event) => {
  try {
    // Send command to voice server to toggle key mode
    const response = await callVoiceServer('/process-manual-command', 'POST', {
      text: 'toggle key mode'
    });
    ...
    return { success: false, error: 'Failed to toggle key mode' };
  } catch (error) {
...
function startCommandPolling() {
  ...
  commandPollingInterval = setInterval(async () => {
    try {
      const response = await callVoiceServer('/get-command-queue', 'GET');
      if (response && response.commands && response.commands.length > 0) {
        ...
        response.commands.forEach(commandData => {
          ...
          // Execute all other commands from queue, including type-text
          if (mainWindow) {
            try {
              const fakeEvent = { sender: mainWindow.webContents };
              executeVoiceCommand(fakeEvent, commandData);
            } catch (e) {
              console.error('Failed to execute queued command:', e);
            }
          }
        });
      }
