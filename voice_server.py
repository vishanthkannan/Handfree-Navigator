...
def process_voice_command(text):
    ...
    # --- Key mode controls ---
    if text in ("key mode", "start key mode", "enable key mode", "key-mode on", "start keyboard mode"):
        is_key_mode = True
        return {
            'action': 'key-mode',
            'command': 'key-mode-on',
    ...
    # If typing mode is enabled, treat free-form speech as text to type
    ...
    if is_key_mode and text:
        ...
        m_combo_any = re.search(r"(shift|ctrl|control|alt)(?:\s*\+|\s+plus\s+|\s+and\s+|\s+)(.+)", text)
        if m_combo_any:
            mod = m_combo_any.group(1)
            raw_key = m_combo_any.group(2).strip()
