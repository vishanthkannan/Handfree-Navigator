...
contextBridge.exposeInMainWorld('voiceAPI', {
  ...
  // Mode toggles (from this morning)
  toggleTypeMode: async () => ipcRenderer.invoke('toggle-type-mode'),
  toggleKeyMode: async () => ipcRenderer.invoke('toggle-key-mode')
});
...
contextBridge.exposeInMainWorld('electronAPI', {
  on: (channel, callback) => {
    // Whitelist channels
    const validChannels = ['show-virtual-keyboard', 'hide-virtual-keyboard', 'toggle-virtual-keyboard', 'voice-command-received', 'display-command', 'mode-status-update'];
